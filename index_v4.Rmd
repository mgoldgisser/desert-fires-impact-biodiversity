...
data_2000 <- filter(data, obsYear > 1999, stzd_totalObs > 0) # filter out anything before 2000 since no NDVI info and no zeros
...
gamm1 <- gam(stzd_totalObs ~ s(ndvi, by = treatmentGroup) + s(desert, ndvi, bs = "re") +
              s(desert, bs = "re") +
              treatmentGroup + 
              obsYear +
              simplified_taxa, family = tw(link = "log"), data = data_2000, method = "REML")

gamm2 <- gam(stzd_totalObs ~ s(ndvi, by = treatmentGroup) + s(desert, ndvi, bs = "re") +
              s(desert, bs = "re") +
              treatmentGroup + 
              obsYear +
              simplified_taxa, family = nb(), data = data_2000, method = "REML")



E2 <- resid(gamm1, type = "pearson")
N <- nrow(data_2000)
p <- length(coef(gamm1))
sum(E2^2) / (N - p)

```
more over-disperson with GAMM

```{r zero-truncated glm, eval=FALSE}

# error message on zero-truncated glm

zt1 <- vglm(formula = stzdTotalObs_rounded ~ ndvi, family = posnegbinomial(), data = data_2000) #error message for some reason
...
zt_pp <- vglm(stzdObs ~ pre_or_post + treatmentGroup + desert + ez_taxa, family = posnegbinomial(), data = data_pp)
