...
ndvi %>% filter(desert == "San Joaquin" & treatmentGroup == "burned") %>% summary()
...
data <- animals %>%
  filter(class != "Aves") %>% #remove aves from data
  group_by(obsYear, treatmentGroup, desert) %>% 
  summarize(totalObs = n()) %>% 
  filter(treatmentGroup != "burned prior 2000")
...
#correlation between yearly no. of fires or size of fires and yearly ndvi

cor_ndvi <- as.data.frame(sites_sf) %>% group_by(desert, FIRE_YEARn) %>% 
  summarize(total_fires = n(), total_area_burned = sum(area_km2)) %>% 
  left_join(ndvi_burned, by = c('FIRE_YEARn' = 'year', "desert" = "desert")) %>% 
  ungroup() %>% 
  select(-c(desert, treatmentGroup))
...
timeseries <- animals %>% filter(treatmentGroup != "burned prior 2000", class != "Aves") %>% 
...
acf_b <- acf(timeseries_b)
acf_con <- acf(timeseries_con)
...
plot(model_gam1, all.terms = TRUE)
...
ndvi_anova  <- aov(ndvi ~ desert + treatmentGroup, data = ndvi)
summary(ndvi_anova)

ndvi_interaction_anova  <- aov(ndvi ~ desert*treatmentGroup, data = ndvi)
summary(ndvi_interaction_anova)

AIC(ndvi_interaction_anova, ndvi_anova)

tuk_ndvi <- TukeyHSD(ndvi_interaction_anova) 
tuk_ndvi
