...
Second graph shows the average number of occurrences reported 5 years before fire and 5 years after fire. Zeros were added to years when no occurrences were reported.

```{r prepost birds only data prep, echo = FALSE}
...
pp_nb1 <- glmmTMB(mean_occurrence ~ pre_or_post + treatmentGroup + (1|desert), family = nbinom1, data = data_pp_5yr)



emm_pp <- emmeans(pp_nb1, ~ treatmentGroup)
...
### overtime
```{r, include=FALSE}
data_ot_aves <- data_ot %>% filter(ez_taxa == "Aves")
hist(data_ot_aves$stzdObs, xlab = "Standardized Occurrences")

ggplot(filter(data_ot_aves), aes(x=yearsSinceFire, y = stzdObs, color = treatmentGroup, shape = simplified_taxa)) +
  geom_point() +
...
ot_nb1 <- glmmTMB(stzdObs ~ treatmentGroup, family = nbinom1, data = data_pp_5yr)
