...
ndvi <- read_csv("data/ndvi.csv") %>% 
  mutate(desert = factor(desert), treatmentGroup = factor(treatmentGroup)) %>% 
  filter(obsYear <= 2020)
...
data <- animals %>%
  filter(obsYear > 1994) %>% 
  group_by(obsYear, ez_taxa, funcGroup, treatmentGroup, desert) %>% 
...
```{r ndvi ANOVA, echo = FALSE}
table(ndvi$desert, ndvi$treatmentGroup)

ndvi_aov2 <- aov(ndvi~desert + treatmentGroup, data = ndvi)

summary(ndvi_aov2)

TukeyHSD(ndvi_aov2, which = "desert")
shapiro.test(ndvi$ndvi)

TukeyHSD(ndvi_aov2, which = "treatmentGroup")

ggplot(ndvi, aes(x = ndvi)) + geom_histogram(aes(fill = desert, color = treatmentGroup)) +
  scale_fill_manual(values = c("#1b9e77", "#d95f02", "#7570b3")) +
  scale_color_manual(values = c("black", "grey75"))

test <- filter(ndvi, desert == "San Joaquin") 
shapiro.test(test$ndvi)
sjd_aov <- aov(ndvi~treatmentGroup, data = test)
summary(sjd_aov)
TukeyHSD(sjd_aov, which = "treatmentGroup")

test <- filter(ndvi, desert != "San Joaquin") 
shapiro.test(test$ndvi)
mojson_aov <- aov(ndvi~treatmentGroup, data = test)
summary(mojson_aov)
TukeyHSD(mojson_aov, which = "treatmentGroup")


test <- filter(ndvi, desert != "San Joaquin", treatmentGroup == "burned") 
shapiro.test(test$ndvi)
mojsonburn_aov <- aov(ndvi~desert, data = test)
summary(mojsonburn_aov)
TukeyHSD(mojsonburn_aov, which = "desert")

test <- filter(ndvi, desert != "San Joaquin", treatmentGroup == "never burned") 
shapiro.test(test$ndvi)
mojsonneverburn_aov <- aov(ndvi~desert, data = test)
summary(mojsonneverburn_aov)
TukeyHSD(mojsonneverburn_aov, which = "desert")
...
ndvi.stats <- ndvi %>% group_by(treatmentGroup, desert) %>% 
  summarize(count = n(), mean = round(mean(ndvi), 2), sd = round(sd(ndvi), 2), SEM2 = round(2 * sd(ndvi)/sqrt(n()), 3))

p1a <- ggplot(ndvi.stats, aes(x = fct_rev(treatmentGroup), y = mean, color = desert)) +
  geom_pointrange(aes(ymin = mean - SEM2, ymax = mean+SEM2, shape = desert), position = position_dodge(width = 0.4)) +
  # geom_point(aes(shape = desert), size = 3, position = position_dodge(width = 0.4)) +
  # geom_errorbar(aes(ymin = mean - SEM2, ymax = mean+SEM2), width = 0.20, position = position_dodge(width = 0.4))  +
  geom_point(aes(x = fct_rev(treatmentGroup), y = ndvi, color = desert), data = ndvi, position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.4), alpha = .3, shape = 1) +
  ylab("Mean Annual NDVI") +
  xlab("") +
  scale_color_manual(values = c("#1b9e77", "#d95f02", "#7570b3")) +
  scale_x_discrete(labels = c("Control", "Burned")) +
  # scale_shape_manual(values = c(16, 17, 18)) +
  scale_y_continuous(limits = c(0, NA), breaks = c(0, .10, .20, 0.30, 0.40)) +
  labs(color = "Desert Ecoregion") +
  guides(shape = "none") +
  theme_bw() + 
  theme(axis.text.y = element_text(size = 12), 
        ...
        plot.margin = margin(0,0.5,0, .5, "cm"))

# ndvi$treatmentGroup <- relevel(ndvi$treatmentGroup, "never burned")
# 
# ndvi_glm <- glm(ndvi~desert + treatmentGroup, data = ndvi)
# summary(ndvi_glm)
# anova(ndvi_glm, test = "Chisq")
# 
# ndvi_emmeans <- emmeans(ndvi_glm, pairwise ~ treatmentGroup|desert)
# summary(ndvi_emmeans)
# test(ndvi_emmeans)
# 
# ndvi_emmeans <- emmeans(ndvi_glm, pairwise ~ desert|treatmentGroup)
# summary(ndvi_emmeans)
# test(ndvi_emmeans)
# 
# ndvi_emmeans <- emmeans(ndvi_glm, pairwise ~ treatmentGroup)
# summary(ndvi_emmeans)
# test(ndvi_emmeans)
...
#######color palette test######
# colors <- pal_aaas("default")(10)
# y = c(rep(10, 10))
# test <- data.frame(colors, y)
# ggplot(test, aes(x = colors, y = y, fill = colors)) +  geom_col() + scale_fill_aaas()
#########################

p1b <- ggplot(ndvi, aes(x = fct_rev(factor(treatmentGroup)), y = ndvi, color = desert)) +
  geom_boxplot(outlier.shape = 18)  +
  # stat_summary computes the statistics summary
  stat_summary(fun.y="mean",color="black", shape=18, aes(group=desert), position=position_dodge(.75), size = 1)+
...
###Old graph that shows means and CI of occ reported; mean and CI manually calculated. Figure updated later for emmeans calc mean and CI
...
#             yrlyAvgORPer1000km2 = ceiling(mean(totalORPer1000km2)),
#             sd_avgORper1000km2 = sd(totalORPer1000km2), SEM = 2*sd_avgORper1000km2/sqrt(n())) %>% 
#   ggplot(aes(x = fct_rev(as.factor(treatmentGroup)), y = yrlyAvgORPer1000km2, shape = desert, color = desert)) +
#   geom_pointrange(aes(ymin = yrlyAvgORPer1000km2-SEM, ymax = yrlyAvgORPer1000km2+SEM), position = position_dodge(width = 0.3)) +
#   ylab(expression('Mean occurrences reported annually per 1,000 '~km^2)) +
#   theme_bw() + 
#   theme(axis.text.y = element_text(size = 12), 
#         axis.text.x = element_text(size = 12),
...
p3 <- data %>% group_by(obsYear, treatmentGroup, ez_taxa, desert) %>% 
  summarize(stzd_Occ = sum(stzd_Occ), totalOcc = sum(totalOcc), totalObs = sum (totalObs)) %>% 
              filter(ez_taxa == "Aves") %>% 
  ggplot(aes(x = obsYear, y = stzd_Occ, color = treatmentGroup)) +
  geom_point(aes(shape = desert), alpha = 0.65)+
  geom_smooth(method = "glm", se = TRUE, aes(fill = treatmentGroup)) +
  ylim(0, NA) +
  xlab("") +
  ylab(bquote('Annual mean occurrences reported per 1,000 ' ~km^2)) +
  theme_bw() +
  scale_color_aaas(labels=c("Control", "Burned area")) +
  scale_fill_aaas() +
...
p4 <- data %>% group_by(obsYear, treatmentGroup, ez_taxa, desert) %>% 
  summarize(stzd_Occ = sum(stzd_Occ), totalOcc = sum(totalOcc), totalObs = sum (totalObs)) %>% 
...
p5 <- data %>% group_by(obsYear, treatmentGroup, ez_taxa, desert) %>% 
  summarize(stzd_Occ = sum(stzd_Occ), totalOcc = sum(totalOcc), totalObs = sum (totalObs)) %>%
  ggplot(aes(x = obsYear, y = stzd_Occ, color = treatmentGroup)) +
  geom_point(aes(shape = desert), alpha = 0.65, size = 3)+
  geom_smooth(method = "glm", se = TRUE, aes(fill = treatmentGroup)) +
  ylim(0, NA) +
  xlab("")  +
  ylab(bquote('Yearly average occurrences reported per 1,000 '~km^2))+
  theme_bw() +
  scale_color_aaas(labels=c("Control", "Burned")) +
  scale_fill_aaas(labels=c("Control", "Burned")) +
  theme(legend.title=element_blank(),
  ...
  #guides(shape = "none") +
  facet_grid(vars(ez_taxa), scales = "free")

plot_grid(p3, p4, rel_widths = c(1,1.3), labels = c("A", "B"), align = "h")

###retired
# plot_grid(p2, p1a, cols = 2, labels = "AUTO", align = "h", axis = "tb")
p5

shapiro.test(test2$totalORPer1000km2)
occ_glm <- glm.nb(totalORPer1000km2~desert * treatmentGroup + obsYear, data = test2)
check_overdispersion(occ_glm)
summary(occ_glm)
anova(occ_glm, test = "Chisq")

```

### Stats
## glm average occurrences reported
```{r glm average occurrence reported all, echo = FALSE, eval = FALSE}
...
test2 <- animals %>% 
  ...
  group_by(treatmentGroup, desert, ez_taxa, obsYear) %>%
  summarize(area_km2 = round(mean(area_km2)),totalOccReported =n(), totalORPer1000km2 = ceiling(sum(totalOccReported/area_km2 * 1000))) 
test2$treatmentGroup <- ifelse(test2$treatmentGroup == "never burned", "control", test2$treatmentGroup)
test2$treatmentGroup <- relevel(as.factor(test2$treatmentGroup), "control")



occ_glm <- glm.nb(totalORPer1000km2~desert * treatmentGroup, data = test2)
check_overdispersion(occ_glm)
summary(occ_glm)
anova(occ_glm, test = "Chisq")

occ_emm <- emmeans(occ_glm, pairwise ~ treatmentGroup)
test(occ_emm)

occ_emm <- emmeans(occ_glm, pairwise ~ desert)
test(occ_emm)
```
```{r glm average occurrence reported birds, echo = FALSE, eval = FALSE}
occA_glm <- glm.nb(totalORPer1000km2~desert * treatmentGroup, data = filter(test2, ez_taxa == "Aves"))
check_overdispersion(occA_glm)
summary(occA_glm)
anova(occA_glm, test = "Chisq")


occA_emm <- emmeans(occA_glm, pairwise ~ treatmentGroup)
test(occA_emm)
confint(occA_emm, level = .95, type = "response")

occA_emm <- emmeans(occA_glm, pairwise ~ desert)
test(occA_emm)
confint(occA_emm, level = .95, type = "response")

occA_emm <- emmeans(occA_glm, pairwise ~ treatmentGroup|desert)
test(occA_emm)
confint(occA_emm, level = .95, type = "response")
```
```{r glm average occurrence reported other, echo = FALSE, eval = FALSE}
occO_glm <- glm.nb(totalORPer1000km2~desert * treatmentGroup, data = filter(test2, ez_taxa == "Other"))
check_overdispersion(occO_glm)
anova(occO_glm, test = "Chisq")

occO_emm <- emmeans(occO_glm, pairwise ~ desert)
test(occO_emm)
confint(occO_emm, level = .95, type = "response")

occO_emm <- emmeans(occO_glm, pairwise ~ treatmentGroup)
test(occO_emm)
confint(occO_emm, level = .95, type = "response")

# graph using emmeans confint
desert <- rep(factor(c("Mojave", "San Joaquin", "Sonoran")), each = 2, times = 2)
treatmentGroup <-rep(factor(c("burned", "control")), times = 6) 
ez_taxa <- rep(factor(c("Aves", "Other")), each = 6)
yrlyAvgORPer1000km2 <- c(8.75, 4.81, 18.81, 13.31, 9.62, 9.04, 3.38, 1.14, 4, 1.29, 19.92, 2.6)
LCL <- c(6.33, 3.45, 12.89, 9.84, 5.51, 6.62, 1.965, .717, 1.711, .789, 14.159, 1.704)
UCL <- c(12.10, 6.71, 27.46, 18, 16.8, 12.33,5.8, 1.82, 9.35, 2.12, 28.02, 3.97)

test_table <- data.frame(desert, treatmentGroup, ez_taxa, yrlyAvgORPer1000km2, LCL, UCL)

  
p2 <- ggplot(test_table, aes(x = fct_rev(as.factor(treatmentGroup)), y = yrlyAvgORPer1000km2, color = desert)) +
  geom_pointrange(aes(ymin = LCL, ymax = UCL, shape = desert), position = position_dodge(width = 0.4)) +
  # geom_point(aes(shape = desert),size = 3, position = position_dodge(width = 0.4)) +
  # geom_errorbar(aes(ymin = LCL, ymax = UCL), width = 0.2, position = position_dodge(width = 0.4)) +
  geom_point(aes(x = fct_rev(as.factor(treatmentGroup)), y = totalORPer1000km2, color = desert), position = position_jitterdodge(jitter.width = 0.075, dodge.width = 0.4), alpha = .3, shape = 1, data = test2) +
  ylab(expression('          Mean occurrences\n reported annually per 1,000 '~km^2)) +
  theme_bw() + 
  theme(axis.text.y = element_text(size = 12), 
        axis.text.x = element_text(size = 12),
        axis.title.x = element_blank(),
        ...
        legend.position = "bottom",
        legend.title=element_blank(),
        plot.margin = margin(0.5,0.1,0, .5, "cm")) +
  scale_color_manual(values = c("#1b9e77", "#d95f02", "#7570b3")) + 
  scale_y_continuous(breaks = c(seq(from = 0, to = 60, by = 15)), limits = c(0, NA))+ 
  scale_x_discrete(labels = c("Control", "Burned"))+
  #scale_shape_manual(values = c(16, 17, 18)) +
  facet_grid(vars(ez_taxa))

plot_grid(p2, p1a, cols = 2, labels = "auto", align = "h", axis = "tb", greedy = TRUE)
```
## occurrences annually time-series
```{r stats occ reported annually w/o ndvi, echo=FALSE}
####glmm####
#### ALL ####

library(car) #to use Anova() walds test 

data <- data %>% group_by(treatmentGroup, obsYear, desert, ez_taxa) %>% 
  summarize(stzd_Occ = sum(stzd_Occ), stzd_totalObs = sum(stzd_totalObs))
...
###all
glmm1_ez <- glmmTMB(stzd_Occ ~ obsYear + treatmentGroup + ez_taxa + (1|desert), 
               data = data, family = "nbinom2")
summary(glmm1_ez)
check_overdispersion(glmm1_ez)

## emmeans
emm_pp <- emmeans(glmm1_ez, pairwise ~ treatmentGroup)
emm_pp <- emmeans(glmm1_ez, pairwise ~ treatmentGroup|ez_taxa)
summary(emm_pp)
...
#### estimate and compare the slopes of fitted lines ####
# 
# occ_trend <- lstrends(glmm1, ~ treatmentGroup, var = "obsYear")
# occ_trend_aves <- lstrends(glmm_aves, ~ treatmentGroup, var = "obsYear")
# occ_trend_other <- lstrends(glmm_other, ~ treatmentGroup, var = "obsYear")

```
```{r COME BACK TO stats occ ~ ndvi, echo = FALSE, eval = FALSE}
...
#### year and NDVI in one model ####
#all
test <- data1 %>%  group_by(obsYear, treatmentGroup, ez_taxa, desert) %>% 
  summarize(totalOcc = sum(totalOcc), totalObs = sum (totalObs), ndvi = mean(ndvi), stzd_Occ = sum(stzd_Occ))

glmm2 <- glmmTMB(stzd_Occ ~ obsYear + ndvi * treatmentGroup + (1|desert) + (1|ez_taxa), 
               data = test,
               family = "nbinom2")
summary(glmm2)
check_overdispersion(glmm2)


emm_pp <- emmeans(glmm2, pairwise ~ treatmentGroup)
summary(emm_pp)

test$fit <- predict(glmm2)

p6 <- ggplot(test, aes(x = obsYear, y = stzd_Occ, group = interaction(desert, treatmentGroup), color = treatmentGroup)) +
  geom_point(aes(shape = desert), alpha = 0.3, size = 2)+
  geom_line(aes(y = fit, lty = desert), size = 0.8) +
  #geom_smooth(method = "glm", se = TRUE, aes(fill = treatmentGroup)) +
  ylim(0, NA) +
  xlab("")  +
  ylab(bquote('Annual mean occurrences reported per 1,000 '~km^2))+
  theme_bw() +
  scale_color_aaas(labels=c("Control", "Burned area")) +
  scale_fill_aaas(labels=c("Control", "Burned area")) +
  theme(legend.title=element_blank(),
        legend.position="bottom",
...
#aves
glmm2_aves <-glmmTMB(stzd_Occ ~ obsYear + ndvi * treatmentGroup + (1|desert),
               data = filter(data1, ez_taxa == "Aves"),
               family = "nbinom1")
summary(glmm2_aves)
check_overdispersion(glmm2_aves)

emm_pp <- emmeans(glmm2_aves, pairwise ~ treatmentGroup)
summary(emm_pp)

#other
glmm2_other <-glmmTMB(stzd_Occ ~ obsYear + ndvi * treatmentGroup + (1|desert),
               data = filter(data1, ez_taxa == "Other"),
               family = "nbinom2")
summary(glmm2_other)
check_overdispersion(glmm2_other)
...
emm_pp <- emmeans(pp_nbAS, pairwise ~ pre_or_post)
test(emm_pp)
...
emm_pp <- emmeans(pp_nbNA, pairwise ~ pre_or_post)
test(emm_pp)
confint(emm_pp, level = .95, type = "response")
...
emm_pp <- emmeans(pp_nbA, pairwise ~ pre_or_post)
test(emm_pp)
...
data_pp_5yrAS.stats <- data_pp_5yrAS %>% group_by(desert, pre_or_post, treatmentGroup, ez_taxa) %>% summarize(mean_observations = ceiling(mean(mean_occurrence)), sd = round(sd(mean_occurrence),2), SEM2 = round(sd/sqrt(n()), 2)) %>% mutate(pre_or_post = factor(pre_or_post, levels = c("pre", "post")))

data_pp_5yrAS.stats1 <- data_pp_5yrAS %>% group_by(pre_or_post, treatmentGroup, ez_taxa) %>% summarize(mean_observations = ceiling(mean(mean_occurrence)), sd = round(sd(mean_occurrence),2), SEM2 = round(sd/sqrt(n()), 2)) %>% mutate(pre_or_post = factor(pre_or_post, levels = c("pre", "post")))


p6 <- ggplot(data_pp_5yrAS.stats1, aes(x = pre_or_post, y =  mean_observations)) +
  geom_point(aes(color = desert, shape = desert), size = 3, data = data_pp_5yrAS.stats)+
  geom_line(aes(group = desert, color = desert), linetype= 1, data = data_pp_5yrAS.stats) +
  #geom_point(color = "black", size = 3, shape = 4)+
  #geom_line(aes(group = ez_taxa),color = "black") +
  geom_errorbar(aes(ymin = mean_observations - SEM2, ymax = mean_observations + SEM2, color = desert), width = 0.1, data = data_pp_5yrAS.stats) +
  scale_x_discrete(labels = c("Before fire", "After fire")) +
  theme(legend.title = element_blank()) +
  xlab("") +
  ylab(bquote("    Mean occurrences \nreported per 10,000 km"^2)) +
  ...
  theme(axis.text.y = element_text(size = 12), 
        ...
        plot.margin = margin(0, 0.1, 0, 0.5, "cm")) +
  facet_grid(vars(ez_taxa), cols = vars(treatmentGroup), scales = "free_y")

# graph using emmeans confint
pre_or_post <- rep(factor(c("pre", "post"), levels = c("pre", "post")), times = 4)
treatmentGroup <-rep(factor(c("burned", "control"), levels = c("control", "burned")), each = 2, times = 2) 
ez_taxa <- rep(factor(c("Aves", "Other")), each = 4)
occ <- c(0.545, 1.560, 15.448, 27.108, 0.133, 0.486, 1.683, 2.998)
LCL <- c(0.183, 0.621, 6.867, 12.304, 0.0474, 0.1927,0.5997, 1.1702)
UCL <- c(1.62, 3.92, 34.75, 59.73, 0.372, 1.225, 4.722, 7.678)


test_table <- data.frame(ez_taxa, treatmentGroup, pre_or_post, occ,  LCL, UCL)

p7 <- ggplot(test_table, aes(x = pre_or_post, y =  occ, color = treatmentGroup)) +
  geom_point(size = 3)+
  geom_line(aes(group = treatmentGroup, color = treatmentGroup), linetype= 1) +
  #geom_point(color = "black", size = 3, shape = 4)+
  #geom_line(aes(group = ez_taxa),color = "black") +
  ...
  xlab("") +
  ylab(bquote("    Mean occurrences \nreported per 10,000 km"^2)) +
  scale_color_manual(labels = c("Control", "Burned"), values = c("#00008b", "#8b0000")) +
  theme_bw() + 
  theme(axis.text.y = element_text(size = 12), 
        axis.text.x = element_text(size = 12),
        axis.title.x = element_text(size = 12),
        ...
        legend.text=element_text(size=11),
        plot.margin = margin(0, 0.1, 0, 0.5, "cm")) +
  facet_grid(rows = vars(ez_taxa), scales = "free_y")


plot_grid(p7, p6, ncol = 2, labels = "auto", align = "h", axis = "tb", greedy = TRUE, rel_widths = c(1, 2))
...
```
```{r pre-post- all aves 5yr avg, echo = FALSE, eval = FALSE}
#mean occurrences 5yrs before and after fire
...
```{r prepost passerines & raptors, echo = FALSE, eval = FALSE}
...
```{r pre-post- pass/rapt 5yr average, echo = FALSE, eval = FALSE}
#mean occurrences 5yrs before and after fire
...
```{r SpadeR ChaoShared, include = FALSE, eval = FALSE}
#Chao Similarity Pair
...
```{r SpadeR ChaoShared ChaoSpecies graph, eval = FALSE}
...
```{r species-specific, echo = FALSE}

table <- animals %>% group_by(species, treatmentGroup, ez_taxa, funcGroup) %>% 
  summarize(totalReports = n()) %>% spread(treatmentGroup, totalReports) %>% 
  rename(control = 'never burned') %>% 
  mutate(burned = ifelse(is.na(burned), 0, burned), 
         control = ifelse(is.na(control), 0, control))

table

ggplot(data_pp, aes(x = factor(pre_or_post, levels = c("pre", "post")), y = stzdObs, fill = treatmentGroup)) +
  geom_col(position = position_dodge()) +
  scale_x_discrete(labels = c("Before fire", "After fire")) +
  theme(legend.title = element_blank()) +
  xlab("") +
  ylab(bquote("Occurrences reported per 10,000 km"^2)) +
  scale_fill_manual(labels = c("Burned", "Control"), values = c('#ff6464', "#6464ff")) +
  theme_bw() +
  theme(axis.text.y = element_text(size = 12),
        axis.text.x = element_text(size = 12),
        axis.title.x = element_text(size = 12),
  ...
  facet_wrap(vars(funcGroup), scales = "free_y")
