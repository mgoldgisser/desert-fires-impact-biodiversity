...
pp_nbAS <- glmmTMB(mean_occurrence ~ pre_or_post*treatmentGroup + (1|ez_taxa) + (1|desert), family = nbinom2, data = data_pp_5yrAS)
...
ggplot(data_pp_5yrAS.stats1, aes(x = pre_or_post, y =  mean_observations)) +
  ...
  theme(legend.title = element_blank()) +
  xlab("") +
  ylab(bquote("Mean observations reported per 10,000 km"^2)) +
  scale_color_manual(values = c("#1b9e77", "#d95f02", "#7570b3")) +
  theme_bw() + 
  theme(axis.text.y = element_text(size = 12), 
        axis.text.x = element_text(size = 12),
...
```{r SpadeR SimPair, include = FALSE, eval = FALSE}
