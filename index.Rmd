...
p1a <- ggplot(ndvi.stats, aes(x = fct_rev(treatmentGroup), y = mean, color = desert)) +
  geom_point(shape = 18, size = 3, position = position_dodge(width = 0.4)) +
  geom_errorbar(aes(ymin = mean - SEM2, ymax = mean+SEM2), width = 0.25, position = position_dodge(width = 0.4))  +
  geom_point(aes(x = fct_rev(treatmentGroup), y = ndvi, color = desert), data = ndvi, position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.4), alpha = .2, shape = 1) +
...
p5 <- data %>% group_by(obsYear, treatmentGroup, ez_taxa, desert) %>% 
  summarize(stzd_Occ = sum(stzd_Occ), totalOcc = sum(totalOcc), totalObs = sum (totalObs)) %>%
  ggplot(aes(x = obsYear, y = stzd_Occ, color = treatmentGroup)) +
  geom_point(aes(shape = desert), alpha = 0.65, size = 2)+
  geom_smooth(method = "glm", se = TRUE, aes(fill = treatmentGroup)) +
  ylim(0, NA) +
  xlab("")  +
  ylab(bquote('Annual mean occurrences reported per 1,000 '~km^2))+
...
```{r glm average occurrence reported all, echo = FALSE}
...
test2 <- animals %>% 
  ...
  group_by(treatmentGroup, desert, ez_taxa, obsYear) %>%
  summarize(area_km2 = round(mean(area_km2)),totalOccReported =n(), totalORPer1000km2 = ceiling(sum(totalOccReported/area_km2 * 1000))) 
test2$treatmentGroup <- relevel(as.factor(test2$treatmentGroup), "never burned")
...
occ_emm <- emmeans(occ_glm, pairwise ~ treatmentGroup|desert)
test(occ_emm)
```
```{r glm average occurrence reported birds, echo = FALSE}
...
```
```{r glm average occurrence reported other, echo = FALSE}
...
occO_emm <- emmeans(occO_glm, pairwise ~ treatmentGroup|desert)
test(occO_emm)
...
p2 <- ggplot(test_table, aes(x = fct_rev(as.factor(treatmentGroup)), y = yrlyAvgORPer1000km2, shape = desert, color = desert)) +
  geom_pointrange(aes(ymin = LCL, ymax = UCL), position = position_dodge(width = 0.3)) +
...
p6 <- ggplot(data_pp_5yrAS.stats1, aes(x = pre_or_post, y =  mean_observations)) +
  ...
  #geom_errorbar(aes(ymin = mean_observations - SEM2, ymax = mean_observations + SEM2), width = 0.1) +
  scale_x_discrete(labels = c("Before fire", "After fire")) +
  theme(legend.title = element_blank()) +
  xlab("") +
  ylab(bquote("    Mean observations \nreported per 10,000 km"^2)) +
  scale_color_manual(values = c("#1b9e77", "#d95f02", "#7570b3")) +
  theme_bw() + 
  theme(axis.text.y = element_text(size = 12), 
        axis.text.x = element_text(size = 12),
...
plot_grid(p7, p6, cols = 2, labels = "auto", align = "h", axis = "tb", greedy = TRUE, rel_widths = c(1, 2))
