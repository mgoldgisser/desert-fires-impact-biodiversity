...
data_birds <- data %>% filter(ez_taxa == "Aves")
data_other <- data %>% filter(ez_taxa == "Other")

anova1 <- aov(stzd_totalObs ~ desert + treatmentGroup, data = data)
anova2 <- aov(stzd_totalObs ~ desert*ez_taxa, data = data)
anova3 <- aov(stzd_totalObs ~ treatmentGroup*ez_taxa, data = data)
anova4 <- aov(stzd_totalObs ~ desert*treatmentGroup, data = data)

AIC(anova1, anova2, anova3, anova4)

anova5 <- aov(stzd_totalObs ~ desert*treatmentGroup, data = data_birds)
anova6 <- aov(stzd_totalObs ~ desert*treatmentGroup, data = data_other)
...
**Tukey for birds: p-value**
1. Mojave:burned-Mojave:never burned 0.9935370  
2. San Joaquin:burned-San Joaquin:never burned  0.0000638  
3. Sonoran:burned-Sonoran:never burned 0.9999994  

+ No significant difference between occurrences reported in Sonoran and Mojave.
+ sig. diff. between burned and control for birds reported in San Joaquin only

***


**Tukey for other taxa: p-value**
1. Mojave:burned-Mojave:never burned 0.9935370  
2. San Joaquin:burned-San Joaquin:never burned  0.0000638  
3. Sonoran:burned-Sonoran:never burned 0.9999994  
...
data_pp <-  data_r %>% filter(n_years <=5) %>% 
...
###old code
# fireYear <- data_pp %>% filter(n_years <=5) %>% {unique(.$fireYear)} %>% sort() %>% rep(.,each=11)

fireYear <- data_pp %>% filter(n_years <=5) %>% group_by(fireYear, desert) %>% summarize(total = n()) %>% select(-total)
...
data_recovery <- rbind(data_pp, test)
...
data_ot <- rbind(data_ot_b, data_ot_c) %>% 
  select(-c(pre_or_post, n_years))
```

```{r, echo =FALSE}

ggplot(data_pp, aes(x = yearsSinceFire, y = totalObs)) +
  geom_point() +
  geom_smooth() +
...
do(data_pp, glance(gam(totalObs ~ yearsSinceFire, data = .)))
...
ggplot(filter(data_recovery, pre_or_post != "same year" & ez_taxa == "Aves"), aes(x = treatmentGroup, y =  stzdObs, fill = factor(pre_or_post, levels = c("pre", "post")))) +
  geom_boxplot() +
...
ggplot(filter(data_recovery, pre_or_post != "same year" & ez_taxa == "Other"), aes(x = treatmentGroup, y =  stzdObs, fill = factor(pre_or_post, levels = c("pre", "post")))) +
  geom_boxplot()+
  scale_fill_aaas(labels = c("Before fire", "After fire")) +
  scale_x_discrete(labels = c("Burned", "Control")) +
  theme(legend.title = element_blank()) +
...
ggplot(filter(data_recovery, pre_or_post != "same year"), aes(x = treatmentGroup, y =  stzdObs, fill = factor(pre_or_post, levels = c("pre", "post")))) +
  geom_boxplot()+
  scale_fill_aaas(labels = c("Before fire", "After fire")) +
  scale_x_discrete(labels = c("Burned", "Control")) +
  theme(legend.title = element_blank()) +
