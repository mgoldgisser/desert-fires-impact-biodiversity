...
hailmary <- c("janb", "rxjrc", "eclong", "Peter_Hall", "yasoykelly", "johngorey", "jrfleullan")
...
animals <- st_read("shapefiles/animals/animals_51022.shp") %>% 
  ...
  mutate(institutionalCode = 
           factor(
             ifelse(recordedBy %in% hailmary, "eButterfly",
                    ifelse(grepl("EBIRD", collection), "CLO-eBird",
                    ifelse(collection == "GBBC", "CLO-GBBC",
                    ifelse(collection == "ML", "CLO-ML",
                    ifelse(collection == "Observations" & is.na(institutionalCode), "Observation.org",
                    ifelse(collection == "Bird sounds", "Xeno-Canto", institutionalCode)))))))) %>% 
...
simplified_taxa <- rep(rep(c("Large Mammals", "Aquatic","Beetles", "Bees", "Tortoises", "Butterflies & Moths", "Frogs and Toads", "Salamanders", "Lizards and Geckos", "Small Mammals", "Birds:Raptors", "Birds:Passerines", "Birds:Woodpeckers", "Birds:Rails and Geese"), each = 6), times = 26)

treatmentGroup <- rep(rep(c("burned", "never burned"), times = 42), times = 26)

desert <- rep(rep(c("Mojave", "San Joaquin", "Sonoran"), times = 28), times = 26)

obsYear <- rep(1995:2020, each = 84)

data2 <- data.frame(obsYear = rep(1995:2020, each = 84),
